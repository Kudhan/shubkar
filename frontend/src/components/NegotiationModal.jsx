            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col overflow-hidden">
                ...
                {booking.status === 'inquiry' || booking.status === 'negotiation' ? (
                    <div className="p-4 bg-white border-t border-gray-100">
                        {/* Quick Actions if NOT my turn (or even if it is, maybe I want to change mind) */}
                        {!isLastMessageMine && messages.length > 0 && (
                            <div className="flex gap-4 mb-4">
                                <button
                                    onClick={handleAccept}
                                    className="flex-1 bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 transition-colors flex justify-center items-center"
                                >
                                    <CheckCircle size={18} className="mr-2" /> Accept Offer
                                </button>
                                <button
                                    onClick={handleReject}
                                    className="flex-1 bg-red-50 text-red-600 py-3 rounded-xl font-bold hover:bg-red-100 transition-colors flex justify-center items-center"
                                >
                        ...
                        <form onSubmit={handleSendOffer} className="space-y-4">
                            <div className="flex gap-4">
                                ...
                                <input
                                    ...
                                    placeholder={messages.length === 0 ? "Add a note with your price..." : "Type a counter message..."}
                                />
                                <button
                                    type="submit"
                                    disabled={loading}
